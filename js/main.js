...
function configSigmaElements(config) {
    ...
    var a = [],
        b,x=1;
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div> IF ' + (x++) + ' (' + sigInst.clusters[b].length + ' TC)</a></div>');
    //a.sort();
...
function showCluster(a) {
    ...
    if (b && 0 < b.length) {
        ...
        $GP.info_data.hide();
        $GP.info_p.html("Topic Cluster im IF:");
        $GP.info_link.find("ul").html(f.join(""));
        $GP.info.animate({width:'show'},350);
        $GP.search.clean();
		$GP.cluster.hide();
